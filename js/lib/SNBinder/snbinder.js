 ...
 * by fintopo
 * 
 * 2015-03-21 項目指定で関数を設定できるようにした
 * 2015-08-22 textareaが「\n」では改行しなくなったため、escapeなし指定でも改行だけ変換するようにした
 * 2015-09-15 関数指定時に引数を付けられるようにした
 */
var SNBinder = (function() {
    ...
    return {
        ...
        noescape: function(text) {  // by fintopo 2015-08-22
          return text.replace(/\n/g, '&#13;');
        },
        compile: function(htm) {
            var _templatize = function(htm) {
                return '"' + htm.replace(/\"/g, "'")
                                ...
                                .replace(/\$\(\_([a-z0-9_\.]*)\)/gi, '"+SNBinder.noescape(""+row.$1)+"') // by fintopo 2015-08-22
                                .replace(/\$\(\:([a-z0-9_\.]*):([a-z0-9_\.]*):([a-z0-9_\.]*)\)/gi, '"+$1(row.$2, $3)+"') // by fintopo 2015-09-15 関数指定時に引数を付けられるようにした
                                .replace(/\$\(\:([a-z0-9_\.]*):([a-z0-9_\.]*)\)/gi, '"+$1(row.$2)+"') // by fintopo 2015-03-21 項目指定で関数を設定できるようにした
                                +'"';
            }; // "
            var _func;
